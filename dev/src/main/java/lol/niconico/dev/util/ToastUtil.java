...
public class ToastUtil {
     ...
     * 短时间提示
     */
    public static void show_short(String content) {
        if (TextUtils.isEmpty(content)) {
            return;
        }
        showToast(Utils.getContext(), null, content);
    }
    /**
     * 短时间提示
     */
    public static void show_short(@StringRes int res) {
        String content = Utils.getContext().getString(res);
        showToast(Utils.getContext(), null, content);
    }
    ...
    private static Toast markToast(Context context, String titleStr, String contentStr, int gravity, int duration) {
        ...
        toast.setGravity(Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, DiUtil.dip2px(100));
